...
exports.getUserWithdrawaltHistory = (userId,trxId,status,limit,offset) => {
    return new Promise((resolve, reject) => {
        ...
        if (trxId) {
            query = `SELECT * FROM f_withdrawal_history A JOIN f_users B ON A.w_user_id = B.uid WHERE A.w_trx_id = '${trxId}' AND A.w_user_id = ${parseInt(userId)} ORDER BY w_wid DESC LIMIT ${limit} OFFSET ${offset}`

        } else if (status) {
            query = `SELECT * FROM f_withdrawal_history A JOIN f_users B ON A.w_user_id = B.uid WHERE A.w_status = ${parseInt(status)} AND A.w_user_id = ${parseInt(userId)} ORDER BY w_wid DESC LIMIT ${limit} OFFSET ${offset}`
        }
        else {
            query = `SELECT * FROM f_withdrawal_history A JOIN f_users B ON A.w_user_id = B.uid WHERE A.w_user_id = ${parseInt(userId)} ORDER BY w_wid DESC LIMIT ${limit} OFFSET ${offset}`
        }
        db.query(query, (err, data) => {
            if (err) reject(err)
            else resolve(data)
