...
exports.updatePackage = (obj,id) => {
    return new Promise((resolve, reject) => {
        db.query("UPDATE f_plans SET ? WHERE plan_id = ?", [obj,parseInt(id)], (err, data) => {
            if (err) reject(err)
            else resolve(data)
        })
    })
...
exports.getPackageById = (packageId) => {
    return new Promise((resolve, reject) => {
        db.query("SELECT * FROM f_plans WHERE plan_status = 1 AND plan_id = ?", parseInt(packageId), (err, data) => {
            if (err) reject(err)
            else resolve(data[0])
        })
    })
...
exports.deletePackageById = (id) => {
    return new Promise((resolve, reject) => {
        db.query("DELETE FROM f_plans WHERE plan_id = ?", parseInt(id), (err, data) => {
            if (err) reject(err)
            else resolve(data)
        })
    })
